...
then
	...
	elif [[ $input =~ "bilivideo.com" ]]
	then
		referer_str="-headers Referer:https://www.bilivideo.com/"
...
# file_format=${t[${#t[*]}-1]}
file_format="mp3"
file_name_sans_extension=${file_name%.*}
echo "file_name_sans_extensions:$file_name_sans_extension"
mp3_file_name="${file_name_sans_extension}.mp3"
echo "mp3 file name:$mp3_file_name, file format:$file_format"
...
for  i  in  ${!a[@]};do
	...
	# fi
	echo "ffmpeg $st_headers $st_user_agent -i $input -ss $left -to $right -q:a 0 -map a $dir_name/clip$i.$file_format" >>run.sh
...
then
   ...
   echo "cp $dir_name/clip0.$file_format \"\`$output\`_cut_$mp3_file_name\"" >>run.sh
else
   echo "multi clip extracts, use concat mode"
   echo "ffmpeg -f concat -i $dir_name/concat.txt -c copy \"\`$output\`_cut_$mp3_file_name\"" >>run.sh
fi
